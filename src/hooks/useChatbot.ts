...
export function useChatbot() {
  ...
  const sendMessage = useCallback(async (content: string, context?: string) => {
    ...
    try {
      ...
      if (content.trim().toLowerCase().startsWith('/agent') && backendBase && /^https?:\/\//i.test(backendBase)) {
        const clean = content.replace(/^\s*\/agent\s*/i, '');
        const url = `${backendBase.replace(/\/$/, '')}/chatbot/agent/command/`;
        const response = await fetch(url, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ message: clean, context: { module: targetContext } }),
        });
        const data = await response.json();
        const text = typeof data?.result?.response === 'string' ? data.result.response : JSON.stringify(data.result || data);
        ...
        addMessage({ type: 'assistant', content: text, context: targetContext });
        setIsTyping(false);
        return;
      }
      ...
      if (backendBase && /^https?:\/\//i.test(backendBase)) {
        try {
          ...
          if (qRes.ok) {
            ...
            const insights = Array.isArray(qData?.insights) ? qData.insights : [];
            const kpiLines = kpis.slice(0, 20).map((k: any) => `- ${k.name}: ${k.current}${k.unit ? ' ' + k.unit : ''} (target ${k.target})`);
            const insightLines = insights.slice(0, 10).map((i: any) => `- ${i.summary}`);
            knowledgePack = [
...
\nUse in-app knowledge (KPIs and insights) and any provided web context to ground your answer. Be precise, cite metrics you use, and tailor recommendations to the user's request and the module.`;
      const combinedContext = [knowledgePack, webContext].filter(Boolean).join('\n\n---\n\n') || null;
      const aiText = await generateAIResponse(currentMessages, { system, webContext: combinedContext });
