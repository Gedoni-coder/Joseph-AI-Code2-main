  ...
  setTimeout(() => {
    ...
    document.querySelectorAll('a[href*="/"]').forEach((el, index) => {
      ...
      if (href && text && !el.hasAttribute("data-joseph-explainable")) {
        makeElementExplainable(el as HTMLElement, `Navigation link: ${text}`, {
          type: "navigation",
          destination: href,
          text: text,
        });
    ...
    document
      .querySelectorAll(".text-2xl, .text-3xl, .text-4xl")
      .forEach((el) => {
        ...
        if (
          ...
          value &&
          value.match(/[\d$%]/) &&
          !parent.hasAttribute("data-joseph-explainable")
        ) {
          const context = parent
            .querySelector(".text-sm, .text-xs")
            ?.textContent?.trim();
          makeElementExplainable(
    ...
    document
      .querySelectorAll('canvas, svg, [class*="chart"], [class*="graph"]')
      .forEach((el) => {
    ...
    document.querySelectorAll('[class*="badge"]').forEach((el) => {
      ...
      if (text && !el.hasAttribute("data-joseph-explainable")) {
        makeElementExplainable(el as HTMLElement, `Status badge: ${text}`, {
          type: "badge",
          status: text,
    ...
    document.querySelectorAll('button[class*="button"], .btn').forEach((el) => {
      ...
      if (!isIcon && text && !el.hasAttribute("data-joseph-explainable")) {
        makeElementExplainable(el as HTMLElement, `Action button: ${text}`, {
          type: "button",
          action: text,
    ...
    console.log(
      "Joseph AI: Demo explainable elements initialized. Click on highlighted elements to get explanations!",
    );
  }, 3000);
}
